"""Reports a `~pydantic.ValidationError` in the vocabulary of the
:file:`technote.toml` file that was validated.
...
``technote.authors.0.name.given``, a ``Value error,`` prefix, a truncated
dump of the rejected input, and a link to pydantic's error documentation.
None of it names anything a technote author wrote.

This module translates such an error back into the file. It walks the same
location against the models it came from and accumulates the address in
TOML's own vocabulary -- the table, the position of the entry counted from
one, the field -- then states each problem as the sentence its validator
raised. It is written against any pydantic model rooted at a TOML file, and
is the same design Documenteer uses to report a :file:`documenteer.toml`, so
both files report their problems the same way.
"""
...
array they are being pointed at. An array not named here falls back to
``item``.
"""
...
def _classify_annotation(
    ...
    ----------
    annotation
        The type annotation of a model field.

    Returns
    -------
    tuple
        The kind of TOML construct the field is written as, and the model its
        contents are validated into, or `None` when they are not a model.
    """
    ...
    if origin is Union or origin is UnionType:
        ...
        return (
            _classify_annotation(members[0]) if members else ("scalar", None)
        )
    if isinstance(annotation, type) and issubclass(annotation, BaseModel):
        return "table", annotation
    if origin in (list, tuple, set, frozenset):
        args = [arg for arg in get_args(annotation) if arg is not Ellipsis]
        return "array", _as_model(args[0] if args else None)
    if origin is dict:
        parameters = get_args(annotation)
        return "mapping", _as_model(parameters[-1] if parameters else None)
    return "scalar", None


def _as_model(annotation: Any) -> type[BaseModel] | None:
    """Return the annotation when it is a pydantic model, and `None`
    otherwise.
    """
    if isinstance(annotation, type) and issubclass(annotation, BaseModel):
        return annotation
...
class _ErrorAddress:
    ...
    zero-based path such as ``technote.authors.0.name.given``, sometimes
    ending in the name of the union member that rejected the value. An author
    reading that has to translate it back into the file they wrote. This
    walks the same location against the models and accumulates the address in
    the file's own vocabulary instead -- the table the error is in, the
    position of the entry counted from one, and the field -- stopping at the
    first segment that names no part of the file, which is how a union
    member's tag is dropped.

    Parameters
    ----------
    root
        The model the file is validated into, which the walk starts from.
    """
    ...
    def walk(self, loc: tuple[int | str, ...]) -> Self:
        ...
        ----------
        loc
            The location of one problem, as pydantic reports it.

        Returns
        -------
        _ErrorAddress
            This address, so that the walk can be chained onto construction.
        """
        segments = list(loc)
        index = 0
    ...
    def _step(self, segment: int | str, following: int | str | None) -> int:
        ...
        if kind == "table":
            ...
            # inside it, so `[technote.license]` names the table that is
            # missing as readily as the one a field of failed.
    ...
    def _name_rejected_key(self, segment: str) -> int:
        ...
        member by -- and ends the walk rather than being printed as though
        the author had written it. A table that forbids extra keys is the
        case where the author *did* write it, and the key is the whole of
        what the error is about, so it is named.
        """
        cursor = self._cursor
...
def _describe_error(
    ...
    -------
    str
        The sentence stating what is wrong.
    """
...
def format_validation_error(
    ...
    source
        The name of the file, such as ``technote.toml``, which the report
        opens by naming.
    ...
    for detail in error.errors(include_url=False):
        place = _ErrorAddress(root).walk(detail["loc"])
        problems.append(
            (
                str(place),
